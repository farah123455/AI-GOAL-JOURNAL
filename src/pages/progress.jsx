...
function computeWeeklyData(journals, goals) {
  ...
  const raw = labels.map((label, i) => {
    ...
    const count = [...journals, ...goals].filter(
      (item) => {
        const d = new Date(
...
export default function Progress() {
   ...
   * This still measures journal + goal activity
   * during the week.
   */
  ...
  const completedActivities =
    goals.length + journals.length;


  /*
   * IMPORTANT:
   * Get real progress from the persisted
   * PostgreSQL Progress records.
   *
   * progress looks approximately like:
   *
   * {
   *   "7": {
   *     id: "2",
   ...
   * }
   */
  const goalProgressValues = goals.map(
    (goal) => {
      ...
      /*
       * First preference:
       * latest record from Progress table.
       */
      if (
        latestProgress?.progress_value !==
        undefined
      ) {
        return Number(
          latestProgress.progress_value
      ...
      /*
       * If a goal is already completed,
       * treat it as 100%.
       */
      if (
      ...
      /*
       * Compatibility fallback in case
       * progress_value exists on the goal.
       */
      return Number(
   ...
   * progress percentage across all goals.
   */
  ...
  /*
   * Recent activity contains journals,
   * goals and latest progress records.
   */
  const recentActivity = [
    ...
    /*
     * Add actual latest progress entries
     * from PostgreSQL to recent activity.
     */
